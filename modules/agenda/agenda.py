from flask import Blueprint, session, redirect, flash, render_template, url_for, jsonify, request
...
def agenda():
    ...
    return render_template('agenda.html', form=form, form_ed=form_ed, data=[ano, mes])
...
def cadastrar_evento():
    ...
    cor = form.cor.data
    novo_evento = Evento(titulo=titulo, descricao=descricao, data_hora=data_hora, completado=False, color=cor, usuario_id=user_id)
    try:
...
def calendario(ano, mes):
    ...
    try:
        ...
        with get_session() as db:
            ...
            eventos = [
                {'id': e.id, 'titulo': e.titulo, 'descricao': e.descricao, 'color': e.color, 'data_hora': e.data_hora.isoformat(), 'completado': e.completado, 'usuario_id': e.usuario_id} for e in eventos
            ]
    except Exception as e:
        print(e)
        flash('Erro inesperado!', 'danger')
